...
public class RepositorioSQL implements IRepositorioInventario {
    ...
    private void guardarEquipo(Equipo e) throws SQLException {
        String sql = "INSERT OR IGNORE INTO Equipos (id, nombre, tipo, descripcion_tecnica) VALUES (?, ?, ?, ?)";
    ...
    public Equipo buscarEquipoPorId(int id) {
        String sql = "SELECT id, nombre, tipo, descripcion_tecnica FROM Equipos WHERE id = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, id);
            ...
            throw new RuntimeException("Error al buscar equipo: " + ex.getMessage());
        }
        return null; 
    }
}